  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = isEditMode
        ? `http://localhost:5000/depositPaymentMethod/deposit-method/${editId}`
        : "http://localhost:5000/depositPaymentMethod/deposit-method";
  ...
  const handleEdit = async (id) => {
    try {
      const response = await fetch(
        `http://localhost:5000/depositPaymentMethod/deposit-method/${id}`
      );
