...
import fishingBG from "../../../assets/table.webp"; // ডামি পাথ, প্রকৃত পাথ দিয়ে রিপ্লেস করো

const Fishing = () => {
  // State for fishing games, loading, error, and selected provider
    ...
    const fetchFishingGames = async () => {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_BASE_API_URL}/games/fishing-all`
        );
        const data = await res.json();
        if (data.success && Array.isArray(data.data)) {
          setFishingGames(data.data);
  ...
  const filteredGames =
    selectedProvider === "সব"
      ? fishingGames
      : fishingGames.filter((game) => game.provider.name === selectedProvider);
