...
import slot_bg from "../../../assets/slot_bg.png"; // ডামি পাথ, প্রকৃত পাথ দিয়ে রিপ্লেস করো

const Slot = () => {
  // State for slot games, loading, error, and selected provider
    ...
    const fetchSlotGames = async () => {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_BASE_API_URL}/games/slot-all`
        );
        const data = await res.json();
        if (data.success && Array.isArray(data.data)) {
          setSlotGames(data.data);
  ...
  const filteredGames =
    selectedProvider === "সব"
      ? slotGames
      : slotGames.filter((game) => game.provider.name === selectedProvider);
