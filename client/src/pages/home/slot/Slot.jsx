...
const Slot = () => {
  ...
  // Filter slot subcategories
  const slotSubcategories = subcategories.filter((sub) => sub.category === "slot");
  ...
  const buttons = [
    ...
    ...slotSubcategories.map((sub) => ({
      image: `${import.meta.env.VITE_BASE_API_URL}${sub?.iconImage}`,
      text: sub.title,
    })),
  ];

  // Filter slot games based on selected subcategory
  const slotGames = games.filter((game) => {
    if (selectedSubcategory === "all") {
      return game.category === "slot";
    }
    return game.category === "slot" && game.subcategory === selectedSubcategory;
  });
